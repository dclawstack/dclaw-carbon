...
### How do I update DClaw Carbon?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp carbon --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Carbon without Kubernetes?

Yes, for local development:

```bash
cd dclaw-carbon/frontend && npm run dev
cd dclaw-carbon/backend && uvicorn dclaw_carbon.main:app --reload
```

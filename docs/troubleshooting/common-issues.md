...
# Check events
kubectl get events -n dclaw-carbon --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-carbon deployment/dclaw-carbon-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-carbon-db-credentials -n dclaw-carbon
   ```
